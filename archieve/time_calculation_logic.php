<?php
/**
 * FAMSS - Time Calculation Logic and Login Handling
 * This file documents the comprehensive time calculation logic used in the FAMSS system,
 * extracted from employee_records.php and logs.php
 */

/**
 * TIME CALCULATION OVERVIEW
 * 
 * The system handles the following time-related calculations:
 ...
 * 5. Automatic lunch break handling
 */

// =================================================================
// 1. AUTOMATIC LUNCH BREAK HANDLING
// =================================================================

/**
 * The system handles lunch breaks in two main ways:
 * 
 * A. For employees who only log AM In and PM Out (skipping AM Out and PM In):
 *    - The system automatically subtracts 1 hour from the total time
 *    - This accounts for the standard lunch break without requiring the actual log entries
 *    
 *    Code implementation:
 ...
 *    }
 * 
 * B. For employees who log AM In and PM Out (even if they log AM Out and PM In):
 *    - The system standardizes the lunch break to 12:00-13:00 (1 hour)
 *    - This ensures consistency in time calculations
 *    
 *    Code implementation:
 *    if (am_in && pm_out) {
 *        amOutVal = '12:00';
...
function computeTotalTime($am_in, $am_out, $pm_in, $pm_out, $department = null, $isHoliday = 0, $isOB = 0, $isSL = 0) {
    ...
    // THIS IS WHERE THE AUTOMATIC LUNCH BREAK IS HANDLED
    $amOutVal = $am_out;
    $pmInVal = $pm_in;
    if ($am_in && $pm_out) {
        $amOutVal = '12:00'; // Standard lunch break start
        $pmInVal = '13:00';  // Standard lunch break end (1 hour)
    }
    ...
    // THIS IS ANOTHER WAY LUNCH BREAK IS HANDLED
    else if (
        $amInMin !== null && $pmOutMin !== null &&
        $amOutMin === null && $pmInMin === null
    ) {
        $total = $pmOutMin - $amInMin - 60; // Subtract 1 hour break automatically
    }
 ...
 * Purpose: Calculates late minutes based on base time
 * 
 * @param string $am_in       Morning time in (format: HH:MM)
...
function calculateLateMinutes($am_in, $department) {
    ...
    $isOtherPersonnel = $department && strtolower(trim($department)) === 'other_personnel';
    $baseHour = $isOtherPersonnel ? 6 : 8; // 6:00 AM for Other_Personnel, 8:00 AM for others
    $baseMinute = 0;
...
function computeUndertime($totalTime, $department, $am_in = null, $isHoliday = 0, $isOB = 0, $isSL = 0) {
    ...
    // Rule 2: If it's OB or SL, no undertime
    if ($isOB == 1 || $isSL == 1) {
        return '';
    }
    
    // Rule 3: If total time is not valid, no undertime
    if ($totalTime === '—') return '';
    ...
    // Rule 4: Adjust base time for undertime calculation based on department
    $isOtherPersonnel = $department && strtolower(trim($department)) === 'other_personnel';

    if ($isOtherPersonnel) {
        // Rule 4.1: For Other_Personnel department
        ...
        // Rule 4.2: For regular employees
        ...
        // Rule 5: Different undertime rules based on total time worked
...
// =================================================================
// 5. SPECIAL CASES - OB, SL, AND HOLIDAYS
// =================================================================
 ...
 * 
 * Implementation in employee_records.php:
 * - OB can be marked through a button in the interface
 * - OB validation prevents marking if SL is already active
 * - OB status is stored in the database and retrieved with attendance records
 */
 ...
 * 
 * Implementation in employee_records.php:
 * - SL can be marked through a button in the interface
 * - SL validation prevents marking if OB is already active
 * - SL status is stored in the database and retrieved with attendance records
 */
 ...
 * 
 * Implementation in employee_records.php:
 * - Holiday status is stored in database and retrieved with attendance records
 * - Holidays are displayed with a special tag in the interface
 */
 ...
 * 
 * Implementation in employee_records.php:
 * - Display times are calculated before rendering:
 *   - let displayAmIn = record.am_in;
 *   - if (isOtherPersonnel && record.am_in) {
 *     - if (h < 6) displayAmIn = '06:00';
 *   - } else if (!isOtherPersonnel && record.am_in) {
 *     - if (h < 8) displayAmIn = '08:00';
 *   - }
 * - AM Out and PM In use default lunch break times if full day present
 */

/**
 * FORMATTED OUTPUT EXAMPLES:
 * 
 * 1. Total Time: "8:00 hrs." or "12:00 hrs." depending on department
 * 2. Late Minutes: "1h 15m (75 mins)" or blank if not late
 * 3. Undertime: "0h 30m (30 mins)" or blank if no undertime
 * 4. Time Display: "8:30 AM" in 12-hour format
 * 5. Date Display: "Jan 15, 2023 (Monday)" with day of week
 */ 